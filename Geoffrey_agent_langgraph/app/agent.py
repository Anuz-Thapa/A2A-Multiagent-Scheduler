...
def generate_geoffrey_calendar() -> dict[str, list[str]]:
    """Generates Geoffrey's calendar for the next 7 days."""
    calendar = {}
    today = date.today()
    # Geoffrey's availability: evenings on weekdays, more free on weekends.
    for i in range(7):
...
GEOFFREY_CALENDAR = generate_geoffrey_calendar()
...
def get_availability(date_range: str) -> str:
    """Use this to get geoffrey's availability for a given date or date range."""
    ...
    try:
        ...
        for i in range(delta.days + 1):
            ...
            available_slots = GEOFFREY_CALENDAR.get(date_str, [])
            if available_slots:
                availability = (
                    f"On {date_str}, geoffrey is available at: "
                    f"{', '.join(available_slots)}."
                )
                results.append(availability)
            else:
                results.append(f"geoffrey is not available on {date_str}.")
...
class GeoffreyAgent:
    """GeoffreyAgent - a specialized assistant for scheduling."""

    SUPPORTED_CONTENT_TYPES = ["text", "text/plain"]

    SYSTEM_INSTRUCTION = (
        "You are geoffrey's scheduling assistant. "
        "Your sole purpose is to use the 'get_availability' tool to answer questions about geoffrey's schedule for playing pickleball. "
        ...
        for item in self.graph.stream(inputs, config, stream_mode="values"):
            ...
            if (
                ...
                yield {
                    ...
                    "content": "Checking geoffrey's availability...",
                }
